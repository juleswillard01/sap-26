...
class GmailAPIReader:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # Load OAuth2 user tokens (generated by tools/gmail_auth.py)
            if self._token_file.exists():
                creds = Credentials.from_authorized_user_file(  # type: ignore[reportUnknownMemberType]
            ...
            if not creds or not creds.valid:
                msg = (
                    f"OAuth2 token invalid or missing: {self._token_file}. "
                    "Run 'uv run python tools/gmail_auth.py' to generate tokens."
                )
