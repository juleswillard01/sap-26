...
5. **Lettrage Semi-Auto** : Scoring confiance pour matching factures ↔ transactions Indy
...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │URSSAFClient  │  │IndyBrowserAdapter│  │PDFGenerator  │  │EmailNotifier │  │
│  │ • OAuth2     │  │ • Playwright  │  │ • WeasyPrint │  │ • SMTP       │  │
...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │API URSSAF    │  │ Web Indy     │  │ Google APIs  │  │ Serveur SMTP │  │
│  │portailapi.   │  │ app.indy.fr  │  │ • Sheets API │  │ (SMTP)       │  │
│  │urssaf.fr     │  │              │  │ • Drive API  │  │              │  │
│  │ • OAuth2     │  │ • Playwright │  │ • v4         │  │              │  │
│  │ • Endpoints  │  │ • Query      │  │              │  │              │  │
│  │   /oauth/token   │ txns        │  │              │  │              │  │
│  │   /particuliers  │             │  │              │  │              │  │
│  │   /demandes-paie │             │  │              │  │              │  │
...
3. RAPPROCHEMENT BANCAIRE (Web/CLI)
   ...
   ├─ IndyBrowserAdapter.get_transactions(date_range)
   │  └─ Playwright : login Indy → naviguer transactions → exporter CSV
...
| **Browser Automation** | Playwright (Python) | 1.40+ | Web scraping Indy : login, navigation transactions, export CSV |
...
sap reconcile [--date-from DATE] [--date-to DATE] [--csv PATH]
    # Lancer lettrage bancaire (Indy → Sheets)
    # --csv PATH : import manuel CSV si automation échoue
...
**Responsabilités** :
- Récupérer transactions Indy (Playwright export)
...
```
Pour chaque facture PAYEE :
  Chercher transactions dans Indy (date_paiement ± 5j)
...
class BankReconciliation:
    async def reconcile(
        ...
        # 1. Fetch transactions from Indy (Playwright) or manual CSV
        if csv_path:
            # Fallback : load from manual export
            transactions = await self.indy_adapter.parse_csv_export(csv_path)
            logger.info(f"Loaded {len(transactions)} from manual CSV export")
        else:
            # Normal flow : Playwright automation
            transactions = await self.indy_adapter.get_transactions(
                date_from, date_to
            )
...
#### 3.4.2 IndyBrowserAdapter (Playwright)

**Responsabilités** :
- Login Indy (email + password)
- Naviguer vers section Transactions
- Exporter transactions (CSV/JSON)
- Parser + mapping vers modèle local
- Error handling + fallback manuel

**Orchestration** :
- Exécution manuelle via CLI : `sap reconcile`
- Ou cron quotidien (APScheduler)
- Utilise MCP (Model Context Protocol) pour isoler la session Playwright
...
class IndyBrowserAdapter:
    ...
    async def get_transactions(
        ...
        """
        Extract transactions from Indy via Playwright.
        Flow:
        ...
        5. Export as CSV
        6. Parse CSV + return list[Transaction]
        """
        try:
    ...
    async def _login(self) -> None:
        """Login to Indy app."""
        await self.page.goto("https://app.indy.fr/login")
        ...
        logger.info(f"Logged in to Indy as {self.email}")
    ...
    async def _export_and_parse(
        ...
        """Navigate to Transactions and export."""
        # Navigate to transactions page
        await self.page.goto("https://app.indy.fr/transactions")

        # Filter by date range (if UI supports)
        await self.page.fill('input[aria-label="From date"]', date_from.isoformat())
        await self.page.fill('input[aria-label="To date"]', date_to.isoformat())
        await self.page.click('button:has-text("Filter")')

        # Wait for table to load
        await self.page.wait_for_selector("table tbody tr", timeout=10000)

        # Export as CSV (click export button)
        await self.page.click('button:has-text("Export")')

        # Download CSV
        async with self.page.expect_download() as download_info:
            await self.page.click('a:has-text("CSV")')
        ...
        logger.info(f"Exported {len(transactions)} transactions from Indy")
        return transactions

    def _parse_csv(self, csv_path: str) -> list[Transaction]:
        """Parse CSV export into Transaction objects."""
        import csv
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                transactions.append(Transaction(
                    ...
                    label=row["Libellé"],
                    status="COMPLETED",  # Indy exports only completed txns
                ))
...
**Fallback Manuel** :
Si Playwright échoue (session expirée, UI changée), Jules peut exporter manuellement :
1. Login Indy (web)
2. Aller à Transactions
3. Exporter CSV
4. Importer via `sap reconcile --csv /path/to/export.csv`

**Configuration** :
```bash
# .env
INDY_EMAIL=jules@example.com
INDY_PASSWORD=secure_password_here
INDY_HEADLESS=true  # false pour debug (affiche navigateur)
```
...
| Transactions | 0s (no cache) | Toujours fresh depuis Indy (daily refresh) |
...
Jules (Web /reconcile OR CLI sap reconcile)
  ...
  │    │
  │    ├─ IndyBrowserAdapter.get_transactions(date_from, date_to)
  │    │  └─ Playwright : login → navigate Transactions → export CSV
  │    │
...
**Temps total** : ~4-6s (Playwright browser launch + Sheets batch ops)
...
└───┬────┘                                                 └────────┬───────┘
    ...
    │              (Playwright: login → export CSV)              │
    │              ← list of transactions from Indy              │
    │                                                               │
...
Transactions : TTL 0             — Always fresh from Indy (daily refresh)
...
**Context** :
Jules utilise Indy comme banque (via app.indy.fr). Source initiale prévoyait API Swan (GraphQL). Nouvelles contraintes :
- ❌ Pas d'accès API Swan (coût/limitations)
- ✅ Indy permet export CSV depuis web
- ✅ Playwright disponible pour automation

**Alternatives** :
1. **Playwright + Headless Browser** (choix) : Automate web login, navigation, CSV export
2. **Indy API officielle** : Non documentée publiquement, possibilité d'accès API dans le futur
3. **Export manuel** : Jules exporte CSV manuellement, importe via CLI (no automation)
4. **Screen scraping** : Regex sur HTML (fragile, maintenance high)

**Decision** :
**Utiliser Playwright pour automation Indy** :
- Login automatisé (email + password)
- Navigation vers Transactions
- Export CSV
- Parse + import dans Sheets

**Justifications** :
- ✅ Zero cost (Playwright open-source)
- ✅ Deterministic automation (réplicable)
- ✅ Fallback manual CSV si Playwright échoue
- ✅ Scalable : run daily cron ou on-demand CLI
- ✅ Isolated session (headless, no impact on production)
- ⚠️ Fragile : dépend UI Indy (peut changer)
- ⚠️ Overhead : browser spin-up ~3-5s

**Consequences** :
- Secret management : INDY_EMAIL + INDY_PASSWORD dans `.env`
- Error handling : capture de screen sur failure pour debug
- Monitoring : timeout 30s, retry 3x with exponential backoff
- Fallback mechanism : CLI support for manual CSV import
- Documentation : Playwright script maintenu comme "living code"

**Risk Mitigation** :
- Email alerts si reconciliation échoue 3x consécutifs
- Manual export fallback documented + tested
- Periodic UI testing (monthly check that selectors work)
- Version lock Playwright (1.40+ tested)
...
| **Indy UI change (Playwright)** | Moyenne | Automation fails | Fallback manual CSV, email alert |
| **Playwright timeout** | Basse | No transaction export | Retry 3x, exponential backoff, timeout 30s |
| **Session expiration (Indy)** | Basse | Login fails | Catch auth errors, screenshot + retry |
...
### Phase 2 (Semaine 2-3)
- [ ] BankReconciliation (Playwright Indy extraction, scoring, lettrage)
- [ ] IndyBrowserAdapter (Playwright automation + CSV fallback)
- [ ] NotificationService + EmailNotifier
- [ ] Dashboard iframes (Sheets pubhtml)
- [ ] Web /reconcile endpoint (support --csv flag)
- [ ] CLI sap reconcile --csv [fallback manual]
- [ ] Reminder email T+36h
- [ ] Integration tests (E2E, including Playwright error scenarios)
...
**SAP-Facture** est conçue comme une **monolith FastAPI asynchrone** avec **Google Sheets comme single source of truth** et **extraction bancaire Indy via Playwright**. Cette architecture est :

✅ **Pragmatique** : zero SQL migrations, formules server-side, coût 0, automation Playwright + fallback manual CSV
✅ **Scalable pour MVP** : 50 factures/mois possible 5+ ans sans migration, transactions from Indy daily
✅ **Sécurisée** : Service Account, input validation, audit logging, credentials in .env
✅ **Maintenable** : Batch operations, caching TTL, error retry logic, IndyBrowserAdapter pattern
✅ **Testable** : Pydantic validation, service injection, unit tests 80%+, Playwright error scenarios

**Réussite mesurée par** :
- Facture créée en < 5s (URSSAF + PDF + Sheets)
- Polling URSSAF 100% completion 4h (no missed invoices)
- Reconciliation Indy daily (< 30s execution time)
