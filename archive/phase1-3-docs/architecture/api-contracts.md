...
class HealthResponse(BaseModel):
    ...
    timestamp: datetime
    services: dict[str, str]  # {google_sheets: ok, urssaf_api: ok, indy_browser: ok}
    version: str
```
...
async def health_check(
    ...
    # Check Indy Browser Adapter
    try:
        # IndyBrowserAdapter.get_account_balance()
        services["indy_browser"] = "ok"
    except Exception:
        services["indy_browser"] = "error"
        status = "degraded"
...
**Responsabilités** : Lettrage automatique factures ↔ transactions Indy (scraping Playwright).
...
class BankReconciliation:
    ...
    async def auto_reconcile(
        ...
        """
        Lettre automatiquement factures PAYEE avec transactions Indy.

        Étapes :
        1. Scrape transactions depuis interface Indy (Playwright) — date range
        2. Importe dans onglet Transactions
...
# Interface IndyBrowserAdapter

**Fichier** : `app/adapters/indy_browser_adapter.py`

**Responsabilités** : Scraping des transactions bancaires via interface web Indy avec Playwright.
...
class IndyBrowserAdapter:
    """Adaptateur Playwright pour scraper transactions bancaires depuis Indy."""
    ...
    def __init__(
        ...
        """
        Initialise le navigateur et crée une session Playwright.

        Args:
            indy_email : str (identifiant Indy)
            indy_password : str (mot de passe Indy)
            headless : bool (mode headless par défaut pour production)

        Raises:
            IndyBrowserAdapterError : si navigateur ne peut être lancé
        """
        self._email = indy_email
        self._password = indy_password
    ...
    async def login(self) -> None:
        """
        Authentifie auprès d'Indy (email/password).

        Étapes :
        1. Lance navigateur Chromium (headless)
        2. Navigate vers indy.fr
        3. Remplir email/password
        4. Attendre redirection dashboard

        Raises:
            IndyBrowserAdapterError : si login échoue ou timeout
        """
        pass
    ...
    async def export_transactions(
        ...
        """
        Scrape transactions depuis le dashboard Indy pour une plage de dates.

        Étapes :
        1. Naviguer vers onglet Transactions
        2. Appliquer filtres date (date_from, date_to)
        3. Scraper tableau (montant, date, libelle, type)
        4. Retourner liste TransactionRow
        ...
        Returns:
            list[TransactionRow] avec indy_transaction_id, montant, libelle, type, etc.

        Raises:
            IndyBrowserAdapterError : si scraping échoue ou timeout
        """
        pass

    async def get_account_balance(self) -> Decimal:
        """
        Récupère solde du compte Indy.

        Returns:
            Decimal : solde en euros

        Raises:
            IndyBrowserAdapterError : si récupération échoue
        """
        pass

    async def close(self) -> None:
        """
        Ferme le navigateur et la session Playwright.
        À appeler lors du shutdown du service.
        """
        pass
```
...
class TransactionRow(BaseModel):
    ...
    indy_transaction_id: str  # ID interne d'Indy depuis scraping Playwright
    date_valeur: str  # YYYY-MM-DD
    montant: Decimal  # Montant en euros (type Decimal pour précision monétaire)
    libelle: str
...
# === Indy Browser Adapter ===

class IndyBrowserAdapterError(SAPFunctureException):
    code = "INDY_BROWSER_ERROR"
    status_code = 502
...
| 502 | Bad Gateway | API URSSAF/Indy Browser/SMTP inaccessible |
...
### Configuration Indy Browser Adapter

| Variable | Type | Requis | Description |
|----------|------|--------|-------------|
| `INDY_EMAIL` | str | Oui | Email de connexion à Indy |
| `INDY_PASSWORD` | str | Oui | Mot de passe Indy (stocké en `.env`, jamais commité) |
| `INDY_HEADLESS` | bool | Non | Mode headless Playwright (défaut: true) |
...
class Settings(BaseSettings):
    """Configuration avec lecture depuis .env"""

    indy_email: str = Field(..., min_length=1)
    indy_password: str = Field(..., min_length=1)
    indy_headless: bool = True
...
settings = Settings()
indy_adapter = IndyBrowserAdapter(
    indy_email=settings.indy_email,
    indy_password=settings.indy_password,
    headless=settings.indy_headless,
)
```
...
INDY_PASSWORD=your-secure-password
INDY_HEADLESS=true
```
